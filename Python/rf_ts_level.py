...
class Node:
        ...
        def fit(self,X,Y, dim_counts, dim_means, dim_unc):
                ...
                for _ in xrange(numVars):
                        ...
                        # if 0 not in dim_counts[self.level]:
                            samples = []
                            for dim in range(dim_counts.shape[1]):
                                if dim_counts[self.level][dim] == 0:
                                    sd = 1.0
                                else:
                                    sd = 1.0 / dim_counts[self.level][dim]
                                picked = np.random.normal(loc=dim_means[self.level][dim], scale=sd)
                        ...
                        for _ in xrange(numIters):
                                ...
                                tempvar, tempT = optimizeW(X, tempdelta, dim_counts, dim_means)
...
class tree:
        ...
        def fit(self,X,Y, dim_counts, dim_means, dim_unc):
                ...
                :param dim_counts = counts of the number of times each dimension has been picked
                :param dim_means = means of each dimension's loss
                ...
                while(queue):
                        ...
                        self.Nodes[currentnode].fit(X[reld],Y[reld], dim_counts, dim_means, dim_unc)
...
def optimizeDelta(x, y, var, d, rl, rr, prc, nrc, plc, nlc) :
        ...
        return delta
def optimizeW(X, Y, dim_counts, dim_means):
        '''
